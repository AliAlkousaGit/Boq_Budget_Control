...
class TestBudgetEngine(FrappeTestCase):
    def setUp(self):
        ...
        self.budget = frappe.get_doc({
            ...
            "categories": [{"category_code": "PRE", "category_name": "Preliminaries",
    ...
    def test_summary_empty(self):
        ...
        cat = self.budget.categories[0].name
        s = get_category_summary(self.budget.name, cat)
    ...
    def test_available_matches_summary(self):
        from boq_budget_control.boq_budget.budget import get_category_summary, get_available
        cat = self.budget.categories[0].name
        _ledger(self.budget, cat, "Reserve", 300)
        self.assertEqual(flt(get_available(self.budget.name, cat)), 700)
    ...
    def test_refresh_summary_writes_columns(self):
        ...
        cat = self.budget.categories[0].name
        _ledger(self.budget, cat, "Reserve", 300)
        _ledger(self.budget, cat, "Actual", 200)
        refresh_summary(self.budget.name)
        row = frappe.db.get_value("BOQ Budget Category", cat, ["reserved_amount", "actual_amount", "available_amount"], as_dict=True)
    ...
    def test_post_ledger_is_idempotent(self):
        ...
        post_ledger("Reserve", self.budget.name, cat, **args)
        post_ledger("Reserve", self.budget.name, cat, **args)  # duplicate -> skipped
        refresh_summary(self.budget.name)
        self.assertEqual(flt(get_available(self.budget.name, cat)), 750)
    ...
    def test_post_reversal_nets_to_zero(self):
        ...
        cat = self.budget.categories[0].name
        post_ledger("Reserve", self.budget.name, cat, voucher_type="Purchase Order", voucher_no="PO-TEST-2", voucher_detail_no="POD-2", amount=400)
        refresh_summary(self.budget.name)
        self.assertEqual(flt(get_available(self.budget.name, cat)), 600)
        post_reversal("Purchase Order", "PO-TEST-2")
        refresh_summary(self.budget.name)
        self.assertEqual(flt(get_available(self.budget.name, cat)), 1000)
    ...
    def test_release_actual_and_cancel(self):
        ...
        cat = self.budget.categories[0].name
        post_ledger("Reserve", self.budget.name, cat, voucher_type="Purchase Order", voucher_no="PO-X", voucher_detail_no="POD-X", amount=500)
        post_ledger("Release", self.budget.name, cat, voucher_type="Purchase Invoice", voucher_no="PI-X", voucher_detail_no="PID-X", amount=500)
        post_ledger("Actual", self.budget.name, cat, voucher_type="Purchase Invoice", voucher_no="PI-X", voucher_detail_no="PID-X", amount=500)
        refresh_summary(self.budget.name)
        s = get_category_summary(self.budget.name, cat)
    ...
    def test_validate_row_stop_blocks(self):
        ...
        cat = self.budget.categories[0].name  # Stop, budget 1000
        self.assertRaises(frappe.ValidationError, validate_row, self.budget.name, cat, 1500)

    def test_validate_row_allow_passes(self):
        from boq_budget_control.boq_budget.budget import validate_row
        self.budget.categories[0].db_set("control_action", "Allow")
        cat = self.budget.categories[0].name
        status, available = validate_row(self.budget.name, cat, 999999, throw=True)
    ...
    def test_reserved_for_po_item(self):
        ...
        cat = self.budget.categories[0].name
        post_ledger("Reserve", self.budget.name, cat, voucher_type="Purchase Order", voucher_no="PO-R", voucher_detail_no="POD-R", amount=700)
        post_ledger("Release", self.budget.name, cat, voucher_type="Purchase Invoice", voucher_no="PI-R", voucher_detail_no="POD-R", amount=200)
